...
class InvitationService {
    ...
    func getMyInvitations(completion: @escaping (Result<[MyInvitationResponse], Error>) -> Void) {
        InvitationService.provider.request(.getMyInvitations) { result in
            switch result {
            case .success(let response):
                switch response.statusCode {
                case 200:
                    do {
                        let baseResponse = try JSONDecoder().decode(BaseResponse<[MyInvitationResponse]>.self, from: response.data)
                        if baseResponse.isSuccess {
